...
type Props = {
  ...
  /** Streaming mode keeps rendering lightweight for smoother transcript scrolling. */
  streaming?: boolean;
};
...
export function ChatMarkdown({ children, citationUrls, streaming = false }: Props) {
  const md = useMemo(() => {
  ...
  if (streaming) {
    return (
      <div className="chat-md whitespace-pre-wrap text-[0.9375rem] leading-relaxed text-[var(--text-primary)]">
        {md}
      </div>
    );
  }
...
export function AssistantChatMarkdown({
  ...
  return (
    <ChatMarkdown
      citationUrls={citationUrls.size > 0 ? citationUrls : undefined}
      streaming={streaming}
    >
      {children}
    </ChatMarkdown>
